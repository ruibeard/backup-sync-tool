...
bool NameMatchesTag(const std::wstring& name, const std::wstring& tag) {
    auto equals_ignore_case = [](const std::wstring& left, const std::wstring& right) {
    ...
    if (equals_ignore_case(name, tag)) {
        return true;
    }

    const size_t colon = name.find(L':');
    return colon != std::wstring::npos && equals_ignore_case(name.substr(colon + 1), tag);
}
...
std::wstring WebDavClient::UrlDecode(const std::wstring& value) const {
    ...
    for (size_t index = 0; index < value.size(); ++index) {
        ...
        if (ch == L'+') {
            ...
            const std::string utf8 = ToUtf8(std::wstring(1, ch));
            bytes.append(utf8);
        }
    }
